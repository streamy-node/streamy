...
class MoviesMgr {
  ...
  async addMovie(movieInfos, fanarts, brickId = null, movieHints) {
    ...
    try {
      ...
      let mediaPath = this.generateMoviePath(
        movieInfos.original_name,
        new Date(movieInfos.release_date)
      );
      let mediaId = await this.con.insertMedia(
      ...
      console.error(
        "Failing adding movie from TheMovieDB",
        movieInfos.original_name,
        e
      );
      this.current_adding_movies.delete(movieInfos.original_name);
  ...
  async addFromMovieDB(movieDBId, brickId = null) {
    ...
    let tmdbInfos = await this.moviedb.movieInfo({
      id: movieDBId,
      language: "en"
    });
    var moviesInfos = {};
    ...
    if (mediaId !== null) {
      ...
      var sql =
        "INSERT INTO `movies_moviedb` (`media_id`,`moviedb_id`)" +
        " VALUES(" +
        mediaId +
        ", " +
        movieDBId +
