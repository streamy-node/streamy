...
class SeriesMgr {
  ...
  async refreshContentTMDB(serieMedia) {
    ...
    if (tmdbId == null) {
      console.error(
        "Cannot refresh media " +
          serieMedia.original_name +
          " with unknown TMDB id"
      );
      return;
    }
    ...
    return await this._refreshContent(
      serieMedia.id,
      tmdbContent.infos,
      tmdbContent.images
    );
  ...
  async _refreshContent(mediaId, serieInfos, serieImages) {
    ...
    // Get database infos
    let dbSerieInfos = await this.mediaMgr.getMediaInfos(
      mediaId,
      langId,
      0,
      null,
    ...
    let dbSeasonsInfos = await this.mediaMgr.getChildrenMediaInfos(
      mediaId,
      langId,
      1,
      null,
    ...
    if (
      serieInfos.langs.en.title.length > 0 &&
      serieInfos.langs.en.overview.length > 0
    ) {
      await this.con.updateMediaTranslation(
        mediaId,
        langId,
        serieInfos.langs.en.title,
    ...
    for (var i = 0; i < serieInfos.seasons.length; i++) {
      ...
      let seasonPath =
        seriePath + "/" + this.generateSeasonSubPath(seasonInfo.season_number);
      ...
      if (dbSeason) {
        ...
        if (seasonInfo.number_of_episodes > dbSeason.number_of_episodes) {
          console.log(
            "Adding new episodes to season " + dbSerieInfos.original_name
          );
          await this.con.updateSeason(
            seasonMediaId,
            dbSeason.season_number,
            seasonInfo.number_of_episodes
          );
        }
      } else {
        // The season is new
        console.log("Adding new season to " + dbSerieInfos.original_name);
        seasonMediaId = await this.con.insertMedia(
        ...
        let seasonId = await this.con.insertSeason(
          seasonMediaId,
        ...
        await this.con.insertMediaTranslation(
          seasonMediaId,
          langId,
          seasonInfo.langs.en.title,
      ...
      for (var j = 0; j < seasonInfo.episodes.length; j++) {
        ...
        let episodePath =
          seasonPath +
          "/" +
          this.generateEpisodeSubPath(episode.episode_number);

        if (dbSeason && dbSeason.children.length > j) {
          //Episode already existing, do nothing for now
        } else {
          //New episode
          let episodeMediaId = await this.con.insertMedia(
          ...
          let episode_id = await this.con.insertEpisode(
            episodeMediaId,
            episode.episode_number
          );
  ...
  async addSerie(serieInfos, serieImages, brickId = null, serieHints) {
    ...
    try {
      ...
      if (
        targetBrickId == null &&
        this.settings.global.new_video_brick === null
      ) {
      ...
      if (
        !("brick_path" in brick) ||
        !(await fsutils.exists(brick.brick_path))
      ) {
        console.error("Video brick not reachable ", brick.brick_path);
        return null;
      }

      let seriePath = this.generateSeriePath(
        serieInfos.original_name,
        new Date(serieInfos.release_date)
      );
      let serieMediaId = await this.con.insertMedia(
        serieInfos.release_date,
        serieInfos.rating,
        serieInfos.rating_count,
        serieInfos.original_name,
        serieInfos.original_language,
        "",
        brick.id,
      ...
      let serieId = await this.con.insertSerie(
        serieMediaId,
        serieInfos.number_of_seasons,
        serieInfos.number_of_episodes
      );
      ...
      for (var i = 0; i < serieInfos.seasons.length; i++) {
        let seasonInfo = serieInfos.seasons[i];
        let seasonName =
          seasonInfo.langs[Object.keys(seasonInfo.langs)[0]].title;
        let seasonPath =
          seriePath +
          "/" +
          this.generateSeasonSubPath(seasonInfo.season_number);
        //let easyName = serieInfos.original_name+" S"+seasonInfo.season_number;
        let seasonMediaId = await this.con.insertMedia(
          seasonInfo.release_date,
          0,
          0,
          seasonName,
      ...
      console.error(
        "Failing adding serie from TheMovieDB",
        serieInfos.original_name,
        e
      );
      this.current_series_creating.delete(serieInfos.original_name);
  ...
  generateEpisodePath(
    ...
    return (
      this.generateSeasonPath(serie_name, serie_release_date, season_number) +
      "/" +
      this.generateEpisodeSubPath(episode_number)
    );
  }
  ...
  generateSeriePath(serie_name, serie_release_date) {
    return (
      "series/" +
      fsutils.cleanFileName(serie_name) +
      " (" +
      serie_release_date.getFullYear() +
      ")"
    );
  }
  ...
  generateSeasonPath(serie_name, serie_release_date, season_number) {
    return (
      this.generateSeriePath(serie_name, serie_release_date) +
      "/" +
      this.generateSeasonSubPath(season_number)
    );
  }
  ...
  async downloadFanarts(mediaId, serieImages) {
    ...
    let topImagesPromises = await this.mediaMgr.downloadFanartsToDir(
      path,
      serieImages
    );
    ...
    for (var i = 0; i < serieImages.seasons.length; i++) {
      ...
      var seasonFolder =
        path + "/" + this.generateSeasonSubPath(season.season_number);
  ...
  async fetchSerieContentFromTMDB(movieDBId) {
    ...
    try {
      ...
      let tmdbInfos = await this.moviedb.tvInfo({
        id: movieDBId,
        langage: "en"
      });
      var serieInfos = {};
      ...
      var serieImages = {};
      serieImages.seasons = [];
      serieImages.fanart500 = this.generateTMDBImageUrl(
        tmdbInfos.poster_path,
        500
      );
      serieImages.fanart300 = this.generateTMDBImageUrl(
        tmdbInfos.poster_path,
        300
      );
      serieImages.fanart200 = this.generateTMDBImageUrl(
        tmdbInfos.poster_path,
        200
      );
      ...
      for (var i = 0; i < tmdbInfos.seasons.length; i++) {
        ...
        let tmdbseasonInfos = await this.moviedb.tvSeasonInfo({
          id: movieDBId,
        ...
        seasonImages.fanart500 = this.generateTMDBImageUrl(
          tmdbseasonInfos.poster_path,
          500
        );
        seasonImages.fanart300 = this.generateTMDBImageUrl(
          tmdbseasonInfos.poster_path,
          300
        );
        seasonImages.fanart200 = this.generateTMDBImageUrl(
          tmdbseasonInfos.poster_path,
          200
        );
        ...
        for (var j = 0; j < tmdbseasonInfos.episodes.length; j++) {
          ...
          var episode_image = {};
          episode_image.fanart200 = this.generateTMDBImageUrl(
            tmdbepisode.still_path,
            200
          );
          episode_image.fanart300 = this.generateTMDBImageUrl(
            tmdbepisode.still_path,
            300
          );
  ...
  async addFromMovieDB(movieDBId, brickId = null) {
    ...
    //Add serie
    let mediaId = await this.addSerie(
      tmdbContent.infos,
      tmdbContent.images,
      brickId
    );
    ...
    if (mediaId !== null) {
      ...
      var sql =
        "INSERT INTO `series_moviedb` (`media_id`,`moviedb_id`)" +
        " VALUES(" +
        mediaId +
        ", " +
        movieDBId +
