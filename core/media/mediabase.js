...
class MediaBase {
  ...
  async getFsMpdFolders(media, absolute = true) {
    let folders = await this.getMediaFsFolders(
      media,
      ["fanart", ".streamy"],
      absolute
    );
  ...
  async getChildrenMediaInfos(mediaId, langId, maxDepth, userId, sortKeyDepth) {
    try {
      ...
      let childrenMedia = await this.con.getMediaChildrenFull(
        mediaId,
        category,
        langId,
        userId,
  ...
  async getMediaInfos(mediaId, langId, maxDepth, userId, sortKeyDepth) {
    try {
      ...
      let mediaFull = await this.con.getMediaFull(
        mediaId,
        categoryId,
        langId,
        userId
      ...
      if (maxDepth >= 1) {
        ...
        let childrenMedia = await this.con.getChildrenMediaInfos(
          mediaId,
          childCategory,
          langId,
          userId,
  ...
  async getMediaListByCategory(
    ...
    let mediaFull = await this.con.getMediaFullList(
      categoryId,
      langId,
      userId,
      sortKey,
      ...
      if (media.parent_id == null) {
        ...
        if (
          !(await fsutils.exists(
            brick.brick_path + "/" + media.path + "/.streamy"
          ))
        ) {
  ...
  async downloadFanartsToDir(absoluteMediaPath, images) {
    try {
      ...
      var downloads = [];
      if ("fanart500" in images && images.fanart500)
        downloads.push(
          netutils.download(
            images.fanart500,
            absoluteMediaPath + "/fanart/img500.jpg",
            false
          )
        );
      if ("fanart300" in images && images.fanart300)
        downloads.push(
          netutils.download(
            images.fanart300,
            absoluteMediaPath + "/fanart/img300.jpg",
            false
          )
        );
      if ("fanart200" in images && images.fanart200)
        downloads.push(
          netutils.download(
            images.fanart200,
            absoluteMediaPath + "/fanart/img200.jpg",
            false
          )
        );
  ...
  async removeMpd(mediaId, mdpFolder) {
    ...
    if (folders.indexOf(mdpFolder) >= 0) {
      //Safe check to remove only mpd dirs
  ...
  async refreshMediaAllMpd(media, recursive = true) {
    ...
    if (recursive) {
      let childMedias = await this.con.getMediaChildrenFull(
        media.id,
        null,
        null,
        null,
  ...
  async removeRepresentation(mediaId, mdpFolder, repId, safeHash = null) {
    try {
      ...
      console.log(
        "Failed to remove representation ",
        mediaId,
        mdpFolder,
        repId,
        err
