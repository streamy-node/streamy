...
class MultiMediaMgr {
  constructor(dbmanager, settings, processesMgr) {
    ...
    this.serieMgr = new SeriesMgr(
      dbmanager,
      settings,
      this.mediaBase,
      this.movieDBMgr
    );
    this.movieMgr = new MoviesMgr(
      dbmanager,
      settings,
      this.mediaBase,
      this.movieDBMgr
  ...
  async removeRepresentation(mediaId, folder, rep_id, safeHash) {
    return this.mediaBase.removeRepresentation(
      mediaId,
      folder,
      rep_id,
      safeHash
  ...
  async getMediaInfos(mediaId, langId, maxDepth, userId, sortKeyDepth) {
    return await this.mediaBase.getMediaInfos(
      mediaId,
      langId,
      maxDepth,
      userId,
  ...
  async getChildrenMediaInfos(mediaId, langId, maxDepth, userId, sortKeyDepth) {
    return await this.mediaBase.getChildrenMediaInfos(
      mediaId,
      langId,
      maxDepth,
      userId,
  ...
  async getMediaListByCategory(
    ...
    return await this.mediaBase.getMediaListByCategory(
      categoryId,
      langId,
      userId,
      orderby,
