  ...
  async postSequential(req) {
    return new Promise((resolve, reject) => {
      ...
      req.busboy.on("file", (fieldname, file, filename) => {
        ...
        this.activeIdentifier.set(identifier, {
          isWritting: true,
          lastChunkNumber: chunkNumber
        });
        const fstream = fs.createWriteStream(
          path.join(self.temporaryFolder, req.body.resumableIdentifier),
          options
        );
        ...
        fstream.on("close", async () => {
          if (numberOfChunks == chunkNumber) {
            ...
            // rename the file
            let uniqueName =
              shortId.generate() + path.extname(original_filename);
            await fsutils.rename(
              this.temporaryFolder + "/" + filename,
              this.targetFolder + "/" + uniqueName
            );
