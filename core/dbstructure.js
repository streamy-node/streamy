...
class DBStructure extends EventEmitter {
  ...
  checkLangCode(langCode) {
    if (langCode.length != 2) {
      console.error(
        "checkLangCode: Invalid lang code provided, " + langCode,
        langCode.length
      );
      return false;
    }
  ...
  async getLangFromIso639_2(langCode) {
    var sql =
      "SELECT languages.* FROM `languages`,`languages_iso_639_2` WHERE languages.id = languages_iso_639_2.language_id AND iso_639_2 = '" +
      langCode +
      "'";
  ...
  async getMediaUsingMpdOnBrick(brickId) {
    var sql =
      "SELECT * FROM `media`" + " WHERE use_mpd = 1 AND brick_id = " + brickId;
    var results = await this.query(sql);
  ...
  async getSerieSeasons(serieId, langCode) {
    ...
    var sql =
      "SELECT * FROM `series_seasons`, `series_seasons_translations`" +
      " WHERE series_seasons.serie_id = " +
      serieId +
      " AND series_seasons.id = season_id AND series_seasons_translations.lang_id = " +
      langCode +
  ...
  getMediaBaseRequest(categoryId, langId, userId, withBrick) {
    ...
    if (langId) {
      ...
      sqlJoins +=
        " LEFT JOIN `media_translations` t  ON m.id = t.media_id AND t.lang_id = " +
        langId; //TODO don't send path to clients
    }
    if (userId) {
      sqlSelect +=
        " p.audio_lang, p.subtitle_lang, p.progression, p.last_seen, p.watched, ";
      sqlJoins +=
        " LEFT JOIN `media_progressions` p ON p.user_id = " +
        userId +
        " AND m.id = p.media_id";
    }
    sqlSelect += " m.* ";
  ...
  async getMediaFullList(
    ...
    if (
      ...
      this.checkOrderBySet(sortKey, this.checkOrderBySet)
    ) {
      console.error("getMedia: Invalid entries ");
      return null;
    }
  ...
  async getMediaFull(mediaId, mediaCategoryId, langCode, userId) {
    try {
      ...
      let sql = this.getMediaBaseRequest(
        mediaCategoryId,
        langCode,
        userId,
        true
  ...
  async getMediaTranslation(mediaId, langCode) {
    ...
    var sql =
      "SELECT * FROM `media_translations`, `languages` WHERE media_id = " +
      mediaId +
      " AND lang_id = " +
      langCode;
    var result = await this.query(sql);
  ...
  async insertMediaTranslation(media_id, lang_id, title, overview) {
    ...
    var sql =
      "INSERT INTO `media_translations` (`media_id`,`lang_id`,`title`,`overview`) " +
      " VALUES( " +
      media_id +
      ", " +
      lang_id +
  ...
  async getSerieSeasonEpisodes(seasonId, langCode) {
    ...
    var sql =
      "SELECT * FROM `series_episodes`, `series_episodes_translations`" +
      " WHERE series_episodes.season_id = " +
      seasonId +
      " AND series_episodes.id = episode_id AND series_episodes_translations.lang_id = " +
      langCode +
  ...
  async getSerieSeasonEpisodesFull(seasonId, langCode, userId) {
    ...
    var sql =
      "SELECT e.*, t.lang_id, t.title, t.overview, p.user_id, p.episode_id, p.audio_lang, p.subtitle_lang, p.progression, p.last_seen FROM `series_episodes` AS e " +
      " LEFT JOIN `series_episodes_translations` AS t  ON e.id = t.episode_id AND t.lang_id = " +
      langCode +
      " LEFT JOIN `users_episodes_progressions` AS p ON p.user_id = " +
      userId +
  ...
  async findSerieFromMoviedbId(movieDBId) {
    ...
    var sql =
      "SELECT media_id FROM series_moviedb " + " WHERE moviedb_id=" + movieDBId;
    let result = await this.query(sql);
    if (result.length > 0) {
  ...
  async findMoviedbIdFromSerie(serieId) {
    ...
    var sql =
      "SELECT moviedb_id FROM series_moviedb " + " WHERE media_id=" + serieId;
    let result = await this.query(sql);
    if (result.length > 0) {
  ...
  async findMovieFromMoviedbId(movieDBId) {
    ...
    var sql =
      "SELECT media_id FROM movies_moviedb " + " WHERE moviedb_id=" + movieDBId;
  ...
  async getMediaPath(mediaId) {
    ...
    var sql =
      "SELECT bricks.path as brick_path, media.original_name as serie_name, series.release_date as serie_release_date, season.season_number,  ep.episode_number" +
      " FROM `series_episodes` AS ep, `series_seasons` AS season, `series`, `bricks` " +
      " WHERE ep.id = " +
      episodeId +
      " and ep.season_id = season.id AND season.serie_id = series.id AND bricks.id = series.brick_id";
    var results = await this.query(sql);
  ...
  async getMdpFiles(mediaId) {
    ...
    var sql =
      "SELECT * FROM `mpd_files` AS mdp " + " WHERE mdp.media_id = " + mediaId;
    var results = await this.query(sql);
  ...
  async getSeriesLiveMdpFiles(episodeId) {
    ...
    var sql =
      "SELECT * FROM `series_live_mpd_files` AS mdp " +
      " WHERE mdp.episode_id = " +
      episodeId;
    var results = await this.query(sql);
  ...
  async getSeriesVideos(mdpId) {
    ...
    var sql =
      "SELECT * FROM `series_videos` AS videos " +
      " WHERE videos.mpd_id = " +
      mdpId;
    var results = await this.query(sql);
  ...
  async getAudios(mdpId) {
    ...
    var sql = "SELECT * FROM `mpd_audios` AS s " + " WHERE s.mpd_id = " + mdpId;
    var results = await this.query(sql);
  ...
  async getSubtitles(mdpId) {
    ...
    var sql =
      "SELECT * FROM `mpd_subtitles` AS s " + " WHERE s.mpd_id = " + mdpId;
    var results = await this.query(sql);
  ...
  async getMpdFile(mpdId) {
    ...
    var sql =
      "SELECT * " + " FROM `mpd_files` as s " + " WHERE s.id = " + mpdId;
    var results = await this.query(sql);
  ...
  async removeMpdFile(mpdId) {
    ...
    let sql = "DELETE FROM `mpd_files`  " + " WHERE id = " + mpdId;
  ...
  async insertMPDFile(media_id, folder, complete, userId = null) {
    ...
    var sql =
      "INSERT INTO `mpd_files` (`media_id`,`folder`,`complete`,`user_id`) " +
      " VALUES(" +
      media_id +
      ",'" +
      folder +
  ...
  async insertMedia(
    ...
    try {
      var sql =
        "INSERT INTO `media` (`release_date`,`rating`,`rating_count`," +
  ...
  async insertVideo(mpd_id, resolution_id) {
    ...
    var sql =
      "INSERT INTO `mpd_videos` (`mpd_id`,`resolution_id`) " +
      " VALUES(" +
      mpd_id +
      ", " +
      resolution_id +
  ...
  async removeMpdVideosInfos(mpd_id) {
    ...
    var sql = "DELETE FROM `mpd_videos` " + " WHERE mpd_id = " + mpd_id + "";
    var sqlres = await this.query(sql);
    return sqlres;
  }
  ...
  async insertAudio(mpd_id, lang_id, lang_subtag_id, channels) {
    ...
    var sql =
      "INSERT INTO `mpd_audios` (`mpd_id`,`lang_id`,`lang_subtag_id`,`channels`) " +
      " VALUES(" +
      mpd_id +
      ", " +
      lang_id +
  ...
  async removeMpdAudioInfos(mpd_id) {
    ...
    var sql = "DELETE FROM `mpd_audios` " + " WHERE mpd_id = " + mpd_id + "";
    var sqlres = await this.query(sql);
    return sqlres;
  }
  ...
  async insertSubtitle(mpd_id, lang_id, lang_subtag_id) {
    ...
    var sql =
      "INSERT INTO `mpd_srts` (`mpd_id`,`lang_id`,`lang_subtag_id`) " +
      " VALUES(" +
      mpd_id +
      ", " +
      lang_id +
  ...
  async removeMpdSubtitleInfos(mpd_id) {
    ...
    var sql = "DELETE FROM `mpd_srts` " + " WHERE mpd_id = " + mpd_id + "";
    var sqlres = await this.query(sql);
    return sqlres;
  }
  ...
  async insertSerie(media_id, number_of_seasons, number_of_episodes) {
    try {
      ...
      var sql =
        "INSERT INTO `media_series` (`media_id`,`number_of_seasons`,`number_of_episodes`) " +
        " VALUES ('" +
        media_id +
        "', " +
        number_of_seasons +
  ...
  async insertSeason(media_id, season_number, number_of_episodes) {
    ...
    var sql =
      "INSERT INTO `media_seasons` (`media_id`,`season_number`,`number_of_episodes`) " +
      " VALUES (" +
      media_id +
      ", " +
      season_number +
  ...
  async insertEpisode(media_id, episode_number) {
    ...
    var sql =
      "INSERT INTO `media_episodes` (`media_id`,`episode_number`) " +
      " VALUES (" +
      media_id +
      ", " +
      episode_number +
  ...
  async getEpisodeId(serieId, seasonNumber, episodeNumber) {
    ...
    var sql =
      ...
      " WHERE mserie.id = " +
      serieId +
      " AND mserie.id = mseason.parent_id AND mseason.id = season.media_id AND  season.season_number =" +
      seasonNumber +
      " AND mseason.id = mepisode.parent_id AND mepisode.id = episode.media_id AND  episode.episode_number = " +
      episodeNumber;
  ...
  async insertMovie(media_id) {
    try {
      ...
      var sql =
        "INSERT INTO `media_movies` (`media_id`) " +
        " VALUES (" +
        media_id +
        ")";
  ...
  async insertAddFileTask(
    ...
    if (
      !this.checkId(media_id) ||
      !this.checkId(brick_id) ||
      working_folder.length == 0 ||
      file.length == 0
    ) {
    ...
    var sql =
      "INSERT INTO `add_file_tasks` (`file`,`brick_id`,`original_name`,`working_folder`,`media_id`,`user_id`) " +
      " VALUES('" +
      file +
      "', '" +
      brick_id +
  ...
  async insertAddFileSubTask(task_id, command, done, output) {
    ...
    var sql =
      "INSERT INTO `add_file_subtasks` (`task_id`,`command`,`done`,`output`) " +
      " VALUES(" +
      task_id +
      ", '" +
      command.replace(/'/g, "\\'") +
  ...
  async getAddFileTaskByFilename(fileName) {
    let sql =
      "SELECT * FROM `add_file_tasks` WHERE file = '" +
      fileName +
  ...
  async removeAddFileTask(id) {
    let sql = "DELETE FROM `add_file_tasks`  " + " WHERE id = " + id;
    let sqlres = await this.query(sql);
    return sqlres;
  }
  ...
  async getAddFileSubTasks(task_id) {
    var sql =
      "SELECT * FROM `add_file_subtasks` WHERE task_id = '" + task_id + "'";
    var results = await this.query(sql);
    return results;
  }
  ...
  async removeAddFileSubTasks(task_id) {
    let sql =
      "DELETE FROM `add_file_subtasks`  " + " WHERE task_id = " + task_id;
    let sqlres = await this.query(sql);
    return sqlres;
  }
  ...
  async getBrickByAliasOrPath(brick_path, alias) {
    var sql =
      'SELECT * FROM `bricks` WHERE `brick_path`="' +
      brick_path +
      '" or `brick_alias`="' +
      alias +
      '"';
  ...
  async insertBrick(brick_alias, brick_path, enabled = 1) {
    if (brick_alias.length == 0 || brick_path.length == 0) {
      ...
      throw new Error(
        "Invalid entries alias:" + brick_alias + " path:" + brick_path
      );
    }
    var sql =
      "INSERT INTO `bricks` (`brick_alias`,`brick_path`,`enabled`) " +
      ' VALUES( "' +
      brick_alias +
      '", "' +
      brick_path +
  ...
  async updateBrickAlias(id, brick_alias) {
    ...
    var sql =
      "UPDATE `bricks` SET `brick_alias` = '" +
      brick_alias.replace(/'/g, "\\'") +
      "' WHERE `id` = " +
      id;
    var sqlres = await this.query(sql);
    var id = sqlres.insertId;
  ...
  async updateBrickStatus(id, enabled) {
    ...
    var sql =
      "UPDATE `bricks` SET `enabled` = " + enabled + " WHERE `id` = " + id;
    var sqlres = await this.query(sql);
    var id = sqlres.insertId;
    return id;
  }
  ...
  async deleteBrick(id) {
    ...
    let sql = "DELETE FROM `bricks` " + " WHERE `id` = " + id;
    let sqlres = await this.query(sql);
    return sqlres;
  }
  ...
  async getTranscodingResolutions(categoryId) {
    var sql =
      "SELECT res.* FROM `" +
      this.categories.get(categoryId) +
      "_transcoding_resolutions` AS tres, `resolutions` AS res " +
  ...
  async getResolutions() {
    var sql = "SELECT * FROM `resolutions` " + " ORDER BY width";
    var results = await this.query(sql);
    return results;
  }

  async getBitrates() {
    var sql = "SELECT * FROM `resolutions_bitrates` " + " ORDER BY bitrate";
    var results = await this.query(sql);
    return results;
  }
  ...
  async getResolutionBitrate(resolutionId) {
    var sql =
      "SELECT * FROM `resolutions_bitrates`, `resolutions` " +
      " WHERE resolution_id = " +
      resolutionId.toString() +
      " AND resolutions.id = resolution_id";
    var results = await this.query(sql);
  ...
  async getBitrate(resolutionId) {
    var sql =
      "SELECT * FROM `resolutions_bitrates` " +
      " WHERE resolution_id = " +
      resolutionId.toString();
    var results = await this.query(sql);
  ...
  async insertWorker(ipv4, port, enabled) {
    try {
      var sql =
        "INSERT INTO `ffmpeg_workers` (`ipv4`,`port`,`enabled`) " +
        " VALUES(INET_ATON('" +
        ipv4 +
        "'), " +
        port +
  ...
  async setWorkerEnabled(ipv4, port, enabled) {
    try {
      var sql =
        "UPDATE `ffmpeg_workers` SET `enabled` = " +
        enabled.toString() +
  ...
  async updateGlobalSettingString(key, value) {
    var sql =
      "UPDATE `global_settings` SET `string` = '" +
      value.replace(/'/g, "\\'") +
      "' WHERE `key` = '" +
      key +
      "'";
  ...
  async updateGlobalSettingInt(key, value) {
    var sql =
      "UPDATE `global_settings` SET `int` = " +
      value +
      " WHERE `key` = '" +
      key +
      "'";
  ...
  async insertUser(username, password, roleId, qosPriority, email, phone = "") {
    var sql =
      "INSERT INTO `users` (`username`,`password`,`role_id`,`qos_priority`,`email`,`phone`) " +
      " VALUES( " +
      "'" +
      username.replace(/'/g, "\\'") +
      "'" +
  ...
  async getUserPassword(userId) {
    var sql = "SELECT password FROM `users` " + " WHERE id = " + userId + "";
    var results = await this.query(sql);
  ...
  async getUserPermissions(userId) {
    var sql =
      "SELECT p.name FROM `permissions` p, `users`, `roles_permissions` rp " +
      " WHERE users.id = " +
      userId +
      "" +
  ...
  async updateUserRole(id, roleId) {
    ...
    try {
      var sql =
        "UPDATE `users` SET `role_id` = " + roleId + " WHERE `id` = " + id;
      var sqlres = await this.query(sql);
      var id = sqlres.insertId;
      return id;
    } catch (err) {
  ...
  async updateUserLastConnection(id) {
    ...
    var sql =
      "UPDATE `users` SET `last_connection` = CURTIME()" +
      " WHERE `id` = " +
      id;
    var sqlres = await this.query(sql);
    var id = sqlres.insertId;
  ...
  async getFfmpegWorkers() {
    var sql =
      "SELECT `id`, INET_NTOA(`ipv4`) as ipv4, `port`, `enabled` FROM `ffmpeg_workers` ";
    var results = await this.query(sql);
    return results;
  }
}
