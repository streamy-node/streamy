    ...
    buildRouter(){
        ...
        router.get('/', loggedIn, async function (req, res) {
            if(req.user){ 
                ...
                let mediaList = await self.multimediaMgr.getMediaListByCategory(self.category, langId, userId, orderby, ascending, count, offset, pattern)
            ...
            if(req.user && req.user.permissions.has("add_media")){ //TODO check rights
                ...
                if(req.body.moviedbId != null){
                    try{
                        ...
                        let mediaId = await self.multimediaMgr.addMediaFromTMDb(self.category,req.body.moviedbId)

                        if(mediaId){
                            res.status(200).send(JSON.stringify({id:mediaId}));
                        }
                    } catch (e) {
                        console.error("Failing adding movie from TMDb ",req.body.moviedbId,e);
