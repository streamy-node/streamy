...
class UploadCtrl{
    ...
    buildRouter(){
        ...
        router.post('/media/:media_id', loggedIn, busMiddleware, async function(req,res){
            var id = parseInt(req.params.media_id);
            
            if(req.user.permissions.has("upload_content")){ //TODO check rights
                if(!id ){
                    res.sendStatus(400);
                    return;
                }
                ...
                if(result.status == 201){
                    ...
                    self.transcodeMgr.addMedia(filename,result.original_filename,parseInt(id));
                }
