...
class MediaCtrl{
    ...
    buildRouter(){
        ...
        router.get('/:mediaId', loggedIn, async function (req, res) {
            ...
            let infos = await self.multiMediaMgr.getMediaInfos(parseInt(mediaId),langId, 0, userId, []);
        ...
        router.post('/:id/mpd/:folderName/refresh', loggedIn, async function (req, res) {
            //let type = req.params.type;
            let id = parseInt(req.params.id);
            let folderName = req.params.folderName;
        ...
        router.delete('/:mediaId/mpd/:folder/representation/:rep_id', loggedIn, async function (req, res) {
            ...
            let result = await self.multiMediaMgr.removeRepresentation(mediaId, folder,rep_id,safeHash)
