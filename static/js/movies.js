class MoviesContent extends ContentController {
  ...
  setup(target) {
    ...
    this.renderHTML(target).ready(function() {
      ...
      self.infiniteScroll.setup(scrollElem, scrollContent, 34, function(
        batchLength,
        lastOffset,
        onResult
      ) {
    ...
    template
      ...
      .attr(
        ...
        "/bricks/" +
          mediaInfos.brick_id +
          "/" +
          encodeURIComponent(mediaInfos.path) +
          "/fanart/img200.jpg"
      );
    template
      .find(".media_title")
      .html(
        mediaInfos.title + " (" + mediaInfos.release_date.substr(0, 4) + ")"
      );
  ...
  renderMovies_elements(seriesInfos) {
    //TODO using only CSS?
  ...
  updateProgressions(progressions) {
    if (location.hash.includes("#movies") && progressions && this.trElements) {
      let relevantMediaIds = Object.keys(progressions.offline).filter(id =>
        this.trElements.has(parseInt(id))
      );
      for (let relevantMediaId of relevantMediaIds) {
        let mediaProgressions = Object.values(
          progressions.offline[relevantMediaId]
        );
