class MediaContentController extends ContentController {
  ...
  renderMpdFolder(mpdInfo) {
    ...
    template
      .find(".mpd_name")
      .attr("href", "#collapse_" + mpdInfo.folder.toString());
    template.find(".mpd_name").append(mpdInfo.folder);
    template
      .find(".panel-collapse")
      .attr("id", "collapse_" + mpdInfo.folder.toString());
    ...
    template.find(".mpd_remove_btn").click(function() {
      ...
      if (r) {
        deleteReq(
          "/media/" + self.mediaId + "/mpd/" + mpdInfo.folder,
          function(response) {
            template.remove();
          },
          function(errReq) {
            alert(
              "Failed to remove content. Reason: " +
                errReq.statusText +
                " " +
                errReq.responseText
            );
    ...
    template.find(".mpd_insert_db_btn").click(function() {
      postAsJson(
        {},
        "/media/" + self.mediaId + "/mpd/" + mpdInfo.folder + "/refresh",
        function(response) {
    ...
    for (var i = 0; i < mpdInfo.mpd.representations.length; i++) {
      ...
      mpd_tpl.find(".rep_remove_btn").click(function() {
        ...
        if (mpdInfo.mpd) {
          r = confirm(
            "You want to remove an representation with data " +
              mpdInfos.contentType
          );
        }
        if (r) {
          ...
          deleteReq(
            "/media/" +
              self.mediaId +
              "/mpd/" +
              encodeURIComponent(mpdInfo.folder) +
              "/representation/" +
              mpdInfos.id +
              "?safe_hash=" +
              mpdInfos.safeHash,
