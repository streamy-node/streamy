class AddMediaItem {
  ...
  render(templateElement, showOverview = false) {
    ...
    //Link search
    this.search = new TmdbSearch(
      template.find(".parsed_name")[0],
      this.type,
      function(mediaInfos) {
  ...
  updateAddFileResults(results, removable = true) {
    ...
    this.element
      ...
      .attr(
        "src",
        theMovieDb.common.images_uri + "original" + results[0].poster_path
      );
    ...
    this.onMediaAdded = function(mediaId) {
      ...
      if (self.file) {
        ...
        if (file.season && file.episode) {
          ...
          console.error(
            "Cannot add a serie with an invalid season or episode number"
          );
          //TODO
        }
      }
