        ...
        for (i = 0; i < arr.length; i++) {
            ...
            if (this.filter || arr[i].substr(0, val.length).toUpperCase() == val.toUpperCase()) {
                ...
                b.addEventListener("click", function(e) {
                    ...
                    let textWithoutDate = this.getElementsByTagName("input")[0].value.replace(/ *\(\d{4}\)/, "");
...
class TmdbSearch{
    ...
    constructor(searchElement,type,onMediaSelected){
        ...
        this.searchField = new Autocomplete(searchElement,[],true,onTitleSelection);
        var bufferedSearch = new BufferedSearch(function(inputData){
            if(inputData.length > 1){
                // Initialize
                self.searchName(inputData,null,
                    function(searchResults){
                        ...
                        for(var i=0; i<8 && i<searchResults.length;i++ ){
                            ...
                            titles.push(name+" ("+new Date(date).getFullYear().toString()+")");   
                        }
